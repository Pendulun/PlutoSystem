...
class TestExpenseService:
    ...
    def test_raises_invalid_num_cols(self, expense_service):
        invalid_num_cols_file = pathlib.Path("tests/_internal/adapters/expense1.csv")
        user_id = '1'
    ...
    def test_raises_empty_first_col(self, expense_service):
        invalid_num_cols_file = pathlib.Path("tests/_internal/adapters/expense2.csv")
        user_id = '1'
        with pytest.raises(ValueError) as e:
            expense_service.add_expense_from_file(invalid_num_cols_file, user_id)
    
    def test_raises_wrong_type_for_amount(self, expense_service):
        invalid_num_cols_file = pathlib.Path("tests/_internal/adapters/expense3.csv")
        user_id = '1'
        with pytest.raises(TypeError) as e:
            expense_service.add_expense_from_file(invalid_num_cols_file, user_id)
    
    def test_dont_raise_for_valid_entries(self, expense_service):
        valid_file = pathlib.Path("tests/_internal/adapters/expense4.csv")
        user_id = '1'
        try:
